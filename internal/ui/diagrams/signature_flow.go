...
func (d *SignatureFlow) Layout(gtx layout.Context, th *theme.Theme) layout.Dimensions {
	...
	shadowBox(gtx, th, i18n.T("diagram.private_key"), image.Pt(inputX, inputY1), inputW, bh, th.Color.WarningBg)
	shadowBox(gtx, th, i18n.T("diagram.message"), image.Pt(inputX, inputY2), inputW, bh, th.Color.InfoBg)

	// Process box: Sign.
	procX := pad + inputW + pct(w, 4)
	procY := inputY1 + (inputY2+bh-inputY1-bh)/2
	shadowBox(gtx, th, i18n.T("diagram.sign"), image.Pt(procX, procY), procW, bh, th.Color.Primary)
	...
	shadowBox(gtx, th, i18n.T("diagram.signature"), image.Pt(outX, procY), outputW, bh, th.Color.TipBg)
	...
	shadowBox(gtx, th, i18n.T("diagram.public_key"), image.Pt(inputX, v1Y), inputW, bh, th.Color.InfoBg)
	shadowBox(gtx, th, i18n.T("diagram.message"), image.Pt(inputX, v2Y), inputW, bh, th.Color.InfoBg)
	shadowBox(gtx, th, i18n.T("diagram.signature"), image.Pt(inputX, v3Y), inputW, bh, th.Color.TipBg)

	// Process box: Verify.
	vprocY := v2Y
	shadowBox(gtx, th, i18n.T("diagram.verify"), image.Pt(procX, vprocY), procW, bh, th.Color.Primary)
	...
	dirArrow(gtx, image.Pt(inputX+inputW, v3Y+bh/2), image.Pt(procX, vprocY+bh/2), lw, lc)

	// Output: Result.
	dirArrow(gtx, image.Pt(procX+procW, vprocY+bh/2), image.Pt(outX, vprocY+bh/2), lw, lc)
	shadowBox(gtx, th, i18n.T("diagram.result"), image.Pt(outX, vprocY), outputW, bh, th.Color.TipBg)
